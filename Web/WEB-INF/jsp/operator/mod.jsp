		...
		function init(_data){
			...
			$('#role_id').combobox({
			    ...
			    onLoadSuccess:function(){
					...
					if (data.length > 0) {
						var role_id_ary=_data.role_id.split(",");
			    		var role_id=[];
			    		for(var i=0;i<role_id_ary.length;i++)role_id.push(role_id_ary[i]);
	        			$('#role_id').combobox('setValues',role_id);
	              	}
				}
			}); 
			closeLoading();
		...
		function save(){
		    ...
		    if(re){
            	$.messager.confirm('提示', '确定保存?', function(r){
					if (r) {
            			saveFunction('/operator/saveMod.action');
					}else{
						return false;
					}
