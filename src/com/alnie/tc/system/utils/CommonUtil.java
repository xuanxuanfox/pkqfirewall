...
public class CommonUtil {
	public static int StatLength(String statStr,int flag)throws Exception {
		int num = 0;// 数字
		...
		else if(flag==1)return letter;
		else return other;
	}
	public static Properties GetWebConfProps(ServletContext context, String path)throws Exception {
		String contextPath = context.getRealPath("/").replace("\\", "/");
		Properties confProps = new Properties();
		confProps.load(new FileInputStream(contextPath + path));
		return confProps;
	}
	public static String GetProConfig(String key)throws Exception {
		Properties pt;
		try {
			pt = GetWebConfProps(Constants.getWebSession().getServletContext(),Constants.SYSTEM_FILE);
	...
	public static String GetHRandomColor()throws Exception{
	   ...
	   String hc = h[R] + h[G] + h[B];
	   return hc;
	}
	public static String asc2gb(String asc)throws Exception {
		String ret;
	...
	public static String gb2asc(String gb)throws Exception {
		String ret;
		if (gb == null)
			return gb;
		try {
	 ...
	 * @return 
	 */  
	public Date GetSwitchDate(String startDate)throws Exception {
	    Date dt=null;  
	...
	public static String GetRoot(String upload_path)throws Exception {
		String root = ServletActionContext.getServletContext().getRealPath(upload_path);
		root = root.replace('\\', '/');
		return root;
	}
	public static void DeleteFile(String root,String storeName)throws Exception {
		File file=new File(root,storeName);
		file.deleteOnExit();
	}
	public static void WriteFile(String file_name,String inValue) throws Exception{
	   	FileWriter fwStart=new FileWriter(file_name,false);
	   	BufferedWriter bwStart=new BufferedWriter(fwStart);
	   	bwStart.write(inValue);
	   	bwStart.close();//关闭文件流
	   	fwStart.close();//关闭文件流
   }
}
